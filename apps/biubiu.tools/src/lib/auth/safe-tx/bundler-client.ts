...
/**
 * 获取 paymaster stub 数据（用于 gas 估算）
 */
export async function getPaymasterStubData(
	...
	const { CHAIN_CONFIG } = await import('./constants.js');
	const chainId = CHAIN_CONFIG[network]?.chainId;
	return rpc('pm_getPaymasterStubData', [
		formatUserOpForRpc(userOp),
		CONTRACTS.entryPoint,
		chainId ? '0x' + chainId.toString(16) : '0x89',
		{ token: tokenAddress }
	], network);
}

/**
 * 获取 paymaster 最终数据（用于提交）
 */
export async function getPaymasterData(
	...
	const { CHAIN_CONFIG } = await import('./constants.js');
	const chainId = CHAIN_CONFIG[network]?.chainId;
	return rpc('pm_getPaymasterData', [
		formatUserOpForRpc(userOp),
		CONTRACTS.entryPoint,
		chainId ? '0x' + chainId.toString(16) : '0x89',
		{ token: tokenAddress }
...
/**
 * 获取 token 报价（包含 paymaster 合约地址用于 approve）
 */
export async function getTokenQuotes(
	network: string,
	tokenAddresses: string[]
): Promise<Array<{ paymaster: string; token: string; postOpGas: string }>> {
	const { CHAIN_CONFIG } = await import('./constants.js');
	const chainId = CHAIN_CONFIG[network]?.chainId;
	return rpc('pimlico_getTokenQuotes', [
