<script lang="ts">
	...
	import { selectedEvent, indexedSlots, type RangeKind } from '$lib/pda-apps/event-scanner/modules/config.js';
	...
	interface Props {
		...
		onSetRange: (patch: Partial<{ kind: RangeKind; days: number; fromDate: string; toDate: string; fromBlock: string; toBlock: string }>) => void;
...
<NetworkPicker
	...
	onSwitchRpc={onSwitchRpc}
	onCustomRpcInput={onSetCustomRpcInput}
	onApplyCustomRpc={onApplyCustomRpc}
/>
...
<section class="card">
	...
	<div class="field">
		...
		<div class="chips">
			{#each PRESETS as p (p.id)}
				<button class="chip" onclick={() => onApplyPreset(p.id)}>{t(`es.preset.${p.labelKey}` as Parameters<typeof t>[0])}</button>
			{/each}
		</div>
	</div>
		...
		{#if cfg.proxy && cfg.proxy.kind !== 'none'}
			<div class="proxy-banner">
				...
				{#if cfg.proxy.implementation}
					<span class="proxy-impl mono">→ {cfg.proxy.implementation.slice(0, 10)}…{cfg.proxy.implementation.slice(-6)}</span>
				{/if}
			</div>
		{/if}
	...
	{#if cfg.events.length > 0}
		<div class="field">
			...
			<select class="input" value={cfg.selectedEventSig} onchange={(e) => onSelectEvent((e.target as HTMLSelectElement).value)}>
		...
		<div class="field">
			<div class="label-row">
				...
				<label class="toggle">
					<input type="checkbox" checked={cfg.walletTrackEnabled} onchange={(e) => onSetWalletTrack((e.target as HTMLInputElement).checked)} />
			...
			{#if cfg.walletTrackEnabled}
				...
				/>
				<p class="hint">{t('es.cfg.trackWalletHint')}</p>
		...
		<div class="field">
			...
			<div class="seg">
				<button class="seg-btn" class:active={cfg.rangeKind === 'lastNDays'} onclick={() => onSetRange({ kind: 'lastNDays' })}>{t('es.cfg.lastDays')}</button>
				<button class="seg-btn" class:active={cfg.rangeKind === 'dates'} onclick={() => onSetRange({ kind: 'dates' })}>{t('es.cfg.dateRange')}</button>
				<button class="seg-btn" class:active={cfg.rangeKind === 'blocks'} onclick={() => onSetRange({ kind: 'blocks' })}>{t('es.cfg.blocks')}</button>
			</div>

			{#if cfg.rangeKind === 'lastNDays'}
				<div class="row">
					<input class="input sm" type="number" min="1" value={cfg.days} oninput={(e) => onSetRange({ days: Number((e.target as HTMLInputElement).value) })} />
					<span class="hint">{t('es.cfg.days')}</span>
					<div class="chips">
						{#each dayChips as d (d)}
							<button class="chip" class:active={cfg.days === d} onclick={() => onSetRange({ days: d })}>{d}d</button>
						{/each}
					</div>
				</div>
			{:else if cfg.rangeKind === 'dates'}
				<div class="row">
					<input class="input sm" type="datetime-local" value={cfg.fromDate} oninput={(e) => onSetRange({ fromDate: (e.target as HTMLInputElement).value })} />
					<span class="hint">→</span>
					<input class="input sm" type="datetime-local" value={cfg.toDate} oninput={(e) => onSetRange({ toDate: (e.target as HTMLInputElement).value })} />
				</div>
			{:else}
				<div class="row">
					<input class="input sm" type="number" min="0" placeholder={t('es.cfg.fromBlock')} value={cfg.fromBlock} oninput={(e) => onSetRange({ fromBlock: (e.target as HTMLInputElement).value })} />
					<span class="hint">→</span>
					<input class="input sm" type="number" min="0" placeholder={t('es.cfg.toBlock')} value={cfg.toBlock} oninput={(e) => onSetRange({ toBlock: (e.target as HTMLInputElement).value })} />
				</div>
			{/if}
		</div>
		...
		<button class="link adv-toggle" onclick={() => (showAdvanced = !showAdvanced)}>
			{showAdvanced ? '▾' : '▸'} {t('es.cfg.options')}
		</button>
		{#if showAdvanced}
			<div class="field adv">
				<label class="toggle">
					<input type="checkbox" checked={cfg.live} onchange={(e) => onSetLive((e.target as HTMLInputElement).checked)} />
					{t('es.cfg.liveTail')}
				</label>
				<input class="input" placeholder={t('es.cfg.scanName')} value={cfg.scanName} oninput={(e) => onSetScanName((e.target as HTMLInputElement).value)} />
				<input class="input mono" placeholder={t('es.cfg.etherscanKey')} value={cfg.etherscanKey} onchange={(e) => onSetEtherscanKey((e.target as HTMLInputElement).value)} />
			</div>
		{/if}
	{/if}
	...
	<!-- Start -->
	<div class="actions">
		<button class="btn btn-primary btn-lg" disabled={!cfg.canExecute || isRunning} onclick={onStart}>
