...
<div class="tab-bar" use:fadeInUp={{ delay: 30 }}>
	...
	<button class="tab" class:active={spaceTab.active === 1} onclick={() => spaceTab.active = 1}>{t('updown5m.space.members')}</button>
	<button class="tab" class:active={spaceTab.active === 2} onclick={() => spaceTab.active = 2}>{isAdmin ? t('updown5m.space.admin') : t('updown5m.space.admin.claimTab')}</button>
</div>
