...
export class RoomDO {
	...
	private onMessage(ws: WebSocket, text: string): void {
		...
		if (role === null) {
			this.send(ws, { t: 'error', code: 'not-joined', message: 'send hello first' });
			return;
		}
		...
		switch (frame.t) {
			...
			case 'bye':
				...
				break;
			default:
				this.send(ws, { t: 'error', code: 'bad-frame', message: 'unknown frame' });
		}
	}
	...
	private onHello(ws: WebSocket, resume?: string): void {
		...
		if (resume) {
			const slot = this.a?.token === resume ? this.a : this.b?.token === resume ? this.b : null;
			if (slot) {
	...
	private onClose(ws: WebSocket): void {
		...
		slot.graceTimer = setTimeout(() => this.dropSlot(role, 'grace-expired'), GRACE_MS);
	}
